...
flow_model = NaN(length(stimuli), 400*400*2*M, 1);
...
for i=1:length(stimuli)
    ...
    while hasFrame(v)  
        ...
        if(mod(frame_index, N/M) == 0 && frame_index>0)
        ...
        flow_movie(:,:,mod(frame_index, N/M)+1) = cat(2, flow.Magnitude, flow.Orientation);
