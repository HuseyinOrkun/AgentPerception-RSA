...
%pth='/Users/huseyinelmas/Desktop/CCN-Lab/data/prior_set_test/';
...
for k=1:length(folders)
	...
	if(folders(k).isdir && ~strcmp(folders(k).name,'.') && ~strcmp(folders(k).name,'..') )
		...
		if(~strcmp(subj_no,'01')) %  && ~strcmp(subj_no,'02') && ~strcmp(subj_no,'03')
			continue;
		end
												  			% Subject folder names are renamed to "subjectXX_" from "subjXX_"
